...
function dessinerCarte(){
    ...
    var map = L.map('carte').setView([50.631672, 3.061784], 15);
...
function placerMarqueurs(map) {
   var l = document.querySelectorAll("table#TableauStations>tbody>tr");
   for (var i=0; i<l.length; i++){
        var texte = text_hover_map(l[i]);
        var lat = l[i].dataset.lat;
        var lon = l[i].dataset.lon;
        var point = [lat, lon];
        var velos = l[i].querySelector("td.velos").textContent;
        var places = l[i].querySelector("td.places").textContent;
        var iconDraw = new VliveImage(velos,places);
        var marqueur = L.marker(point, {icon:iconDraw.getLeafletIcon()}).addTo(map)
        .bindPopup(texte);
        ...
        marqueur.on("click", function(e){
          window.open("infos.php?lat="+e.latlng.lat+"&lng="+e.latlng.lng, "_self");
        });
    }
  }

function text_hover_map(ligne){
  var nom = ligne.querySelector("td.nom").textContent;
  var commune = ligne.querySelector("td.commune").textContent;
  return "<p>"+nom+"</p><p>"+commune+"</p>";
}
