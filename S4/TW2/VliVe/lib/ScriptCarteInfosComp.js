/* Léane Texier */
window.addEventListener("load",dessinerCarteComp);


function dessinerCarteComp(){
    /* Recuperation des données nécessaires pour la carte */
    var lat = document.querySelector("p.lat").textContent;
    var lng = document.querySelector("p.lng").textContent;
    var velos = document.querySelector("p.velos").textContent;
    var places = document.querySelector("p.places").textContent;
    /* Point correspondant à la station */
    var point = [lat, lng];
    /* Creation de la carte */
    var map = L.map('carteInfos').setView(point, 18);
    L.tileLayer('http://{s}.tile.osm.org/{z}/{x}/{y}.png', {
        attribution: '©️ <a href="http://osm.org/copyright">OpenStreetMap</a> contributors'
    }).addTo(map);
    /*Marqueur*/
    var iconDraw = new VliveImage(velos,places);
    L.marker(point, {icon:iconDraw.getLeafletIcon()}).addTo(map);
};
