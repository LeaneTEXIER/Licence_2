...
def write_occurrences(occurrences, filename):
    ...
    for occ in occurrences:
        stream.write(bytes(occ)+bytes(struct.pack('i',occurrences[occ])))
    stream.close()
...
def read_occurrences(filename):
    ...
    byte=stream.read(1)
    while byte!=b'':
        occurrences[byte]=struct.unpack('i',stream.read(struct.calcsize('i')))[0]
        byte=stream.read(1)
    stream.close()
    return occurrences
...
def huffman_encode(filename, out_filename):
    ...
    occurrences=symbol_occurrences(stream)
    # Création du codage de Huffman
    cod_Huff=huffman_coding(create_huffman_tree(occurrences))
    # Ecriture du fichier stockant le dictionnaire d'occurrences
    write_occurrences(occurrences,out_filename +".code")
    ...
    newFile=''
    while len(byte)!=0:
        newFile+=cod_Huff.code(byte)
        byte=stream.read(1)
    tp4.write_binary_string_in_file(newFile,out_filename)
    stream.close()
...
def huffman_decode(filename, out_filename):
    ...
    Huff=create_huffman_tree(occurrences)
    aux=prefix_tree_decoding(tp4.read_file(filename),Huff)
    out_stream = open(out_filename, 'wb')
    out_stream.write(aux)
    out_stream.close()
